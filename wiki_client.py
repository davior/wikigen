...
class WikiClient:
    ...
    def get_changes_since(self, since_rcid: int, limit: int = 500) -> set[str] | None:
        """Return the set of main-namespace titles changed since ``since_rcid``.

        Includes move targets (so a rename surfaces both old and new titles).
        Returns None when the gap is larger than ``limit`` changes — the caller
        should fall back to a full rebuild rather than a partial delta.
        """
        self._ensure_connected()
        r = self._session.get(self._url, params={
            'action': 'query', 'list': 'recentchanges',
            'rcnamespace': 0, 'rclimit': limit,
            'rcprop': 'title|ids|loginfo', 'rctype': 'edit|new|log',
            'format': 'json',
        })
        r.raise_for_status()
        changes = r.json().get('query', {}).get('recentchanges', [])
        ...
        reached = False
        for ch in changes:
            if ch.get('rcid', 0) <= since_rcid:
                reached = True
                break
            if ch.get('title'):
                titles.add(ch['title'])
            target = (ch.get('logparams') or {}).get('target_title')
            if target:
                titles.add(target)
        if not reached and len(changes) >= limit:
            return None  # too many changes to delta cheaply — caller rebuilds
        return titles

    def get_categories_for_titles(self, titles) -> dict[str, list[str] | None]:
        """Return {title: [categories]} for given titles; None value = page missing.

        Used to refresh only the entries that changed, instead of re-scanning the
        whole wiki. Missing pages (deleted or moved away) map to None.
        """
        self._ensure_connected()
        titles = list(titles)
